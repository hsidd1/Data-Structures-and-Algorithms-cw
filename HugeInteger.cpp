...
std::string HugeInteger::karatsuba(std::string left_side, std::string right_side) {
	...
	//Computes four intermediate products: p0 = lhs0 * rhs0, p1 = lhs1 * rhs1, p2 = (lhs0 + lhs1) * (rhs0 + rhs1), and p3 = p2 - (p0 + p1).
	std::string prod1 = karatsuba(left_first, right_first);
	std::string prod2 = karatsuba(left_second, right_second);
	// std::string p2 = karatsuba(add(lhs0,lhs1),add(rhs0,rhs1));
	std::string prod3 =
   ...
   // std::string p3 = ((HugeInteger(p0).add(HugeInteger(p1))))
	//                     .subtract(HugeInteger(p2))
	 //                    .toString();
